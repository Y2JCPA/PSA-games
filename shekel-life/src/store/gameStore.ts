...
const createInitialLevelState = (id: LevelId): LevelState => ({
  id,
  status: 'unlocked',
  balance: 0,
  totalEarned: 0,
...
const initialState: GameState = {
  language: 'en',
  levels: {
    1: createInitialLevelState(1),
    2: createInitialLevelState(2),
    3: createInitialLevelState(3),
    4: createInitialLevelState(4),
  },
...
export const useGameStore = create<GameState & GameActions>()(
  persist(
    ...
    {
      name: 'shekel-life-game',
      storage: createJSONStorage(() => AsyncStorage),
      // Migrate old locked states to unlocked
      merge: (persisted, current) => {
        const merged = { ...current, ...(persisted as object) };
        // Ensure all levels are unlocked (migration from locked → open)
        if (merged.levels) {
          for (const id of [1, 2, 3, 4] as LevelId[]) {
            if (merged.levels[id] && merged.levels[id].status === 'locked') {
              merged.levels[id] = { ...merged.levels[id], status: 'unlocked' };
            }
          }
        }
        return merged as GameState & GameActions;
      },
    }
  )
);
